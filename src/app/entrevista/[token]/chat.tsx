...
export function Chat({
  ...
  progresoInicial: ProgresoBloque[];
}) {
  const [mensajes, setMensajes] = useState<Mensaje[]>(mensajesIniciales);
  const [progreso, setProgreso] = useState<ProgresoBloque[]>(progresoInicial);
  ...
  async function enviar() {
    ...
    try {
      ...
      if (cuerpo.progreso) setProgreso(cuerpo.progreso);
    } catch {
  ...
  return (
    <div className="flex flex-1 flex-col sm:flex-row">
      ...
      <div className="flex flex-1 flex-col bg-zinc-50 dark:bg-black">
        ...
        {progreso.length > 0 && progreso.every((b) => b.completo) && (
          <div className="border-t border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-black">
            <Link
              href={`/entrevista/${token}/confirmacion`}
              className="flex h-11 w-full items-center justify-center rounded-full bg-foreground px-5 text-sm font-medium text-background"
            >
              Ya tengo todo — revisar y confirmar mis datos
            </Link>
          </div>
        )}
